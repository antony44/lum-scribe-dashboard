...
export default function StatCard({ value, label, trend, color, badge, tooltip, children }: StatCardProps) {
  return (
    <div className="flex flex-col bg-white dark:bg-[#161C24] rounded-xl border dark:border-[#202837]/50 px-5 py-4 shadow-sm min-w-[110px] max-w-[170px] flex-1 justify-between">
      <div className="flex items-center">
        <span className="text-2xl font-bold mr-2 dark:text-white">{value}</span>
        {badge && (
          <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-xs font-semibold rounded px-2 py-0.5 ml-1">
            {badge}
          </span>
        )}
        {trend && (
